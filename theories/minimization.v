...
Section Isomopism.
  ...
  Proof using A_coll A_conn B_coll B_conn L_AB.
    exists iso. split.
  ...
  Proof using. move => [iso [H _ _ _]]. exact (bij_card H). Qed.
End Isomopism.
...
Section Prune.
  ...
  Proof using. apply/dec_eq. exists nil. exact: eqxx. Qed.
  ...
  Proof using. by rewrite card_sig subset_leq_card // subset_predT. Qed.
...
Section Collapse.
  ...
  Proof using. move => [H1 H2]. by rewrite /coll_fun !delta_cat H1 H2. Qed.
  ...
  Proof using. apply/collP. rewrite /coll. auto. Qed.

  Lemma collb_sym : symmetric collb.
  Proof using. move => x y. by apply/collP/collP; move => H w; rewrite H. Qed.

  Lemma collb_trans : transitive collb.
  Proof using. move => x y z /collP H1 /collP H2. apply/collP => w. by rewrite H1 H2. Qed.

  Lemma collb_step a x y : collb x y -> collb (dfa_trans x a) (dfa_trans y a).
  Proof using. move => /collP H. apply/collP => w. by rewrite !delta_cons H. Qed.
...
Proof using. rewrite card_sub. exact: max_card. Qed.
...
Proof using. exact: leq_trans (collapse_size _) (dfa_prune_size _). Qed.

Lemma minimize_collapsed (A : dfa) : collapsed (minimize A).
Proof using. exact: collapse_collapsed. Qed.

Lemma minimize_connected (A : dfa) : connected (minimize A).
Proof using. apply collapse_connected. exact: dfa_prune_connected. Qed.

Lemma minimize_correct (A : dfa) : dfa_lang (minimize A) =i dfa_lang A.
Proof using. move => x. by rewrite collapse_correct dfa_prune_correct. Qed.

Hint Resolve minimize_size minimize_collapsed minimize_connected : core.

Lemma minimize_minimal A : minimal (minimize A).
Proof using. apply: abstract_minimization => B. auto using minimize_correct. (* and hints *) Qed.
...
Proof using. move => L_AB /minimalP [? ?] /minimalP [? ?]. exact: coll_iso. Qed.
